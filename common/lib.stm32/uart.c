...
void TestSend()
{
	u1Fifo.txBuf[0]='A';
	u1Fifo.txBuf[1]='B';
	uart4TxDMAConfiguration(UART4, UART4_TX_DMA, u1Fifo.txBuf, 2,1);
}

void uart4TxDMAConfiguration(USART_TypeDef *uart, DMA_Channel_TypeDef *dmatx,
 ...
 * @retval : None
 */
void uart4RxDMAConfiguration(USART_TypeDef *uart, DMA_Channel_TypeDef *dmarx,
...
//	GPIO_PinRemapConfig(GPIO_PartialRemap_UART4,ENABLE);
	...
	USART_ClearFlag(UART4, USART_FLAG_CTS | USART_FLAG_LBD  |
						USART_FLAG_TC  | USART_FLAG_RXNE );

	uart4RxDMAConfiguration(UART4, UART4_RX_DMA, u1Fifo.rxBuf,UARTRX_FIFO_SIZE);
...
//      USART_ITConfig(UART4, USART_IT_RXNE, ENABLE);
	    /* Configure UART4 interrupt */
	      NVIC_SetPriority(UART4_RX_DMA_IRQn, 0x7);
		  NVIC_EnableIRQ(UART4_RX_DMA_IRQn);
		/* Enable the USART3 */
	/* Enable the UART4 */
...
unsigned int uart3FindEscapedPack()
{
	...
	while (i != (UARTRX_FIFO_SIZE - DMA_GetCurrDataCounter(UART4_RX_DMA)))
	{
		...
		if (u1Fifo.rxBuf[i] == 0x55)
		{
			...
			if (i == (UART4_RX_DMA->CMAR - (uint32_t) &u1Fifo.rxBuf[0]))
...
{
	if (u1Fifo.rxCurrent != (UARTRX_FIFO_SIZE - DMA_GetCurrDataCounter(UART4_RX_DMA)))
	{
...
void uart3TxTask()
{
	unsigned int cnt = 0;
  // If DMA COMPLITE
	if ((UART4_TX_DMA->CCR & ((uint32_t) 0x00000001)) == 0)
	{
				...
				< UARTTX_FIFO_SIZE))
	{
		...
		if (cnt > 0)
		{
			uart4TxDMAConfiguration(UART4, UART4_TX_DMA, u1Fifo.txBuf, cnt,	1);
			DMA_ITConfig(UART4_TX_DMA, DMA_IT_TC, ENABLE);
           // }
				/* Enable USART_Tx DMA Tansmit request */
				/* Configure DMA1_Channel_Tx interrupt */
				NVIC_SetPriority(UART4_TX_DMA_IRQn, 0x06);
				NVIC_EnableIRQ(UART4_TX_DMA_IRQn);
		}
	}
}
//-----------------------------------------------------------------------------------
...
//	DMA_Cmd(UART4_TX_DMA, DISABLE);
	...
	u1Fifo.rxCurrent+=0;
	int tempTest = DMA_GetCurrDataCounter(UART4_RX_DMA);
			tempTest +=0;
	//DMARxDataReceived();
	//uart3TxTask();
}
